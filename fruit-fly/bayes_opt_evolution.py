...
# helpers
def generate_proj(PN_SIZE, NUM_PROJ, WTA_DIM):
  """
...
def init_pop(POP_SIZE, PN_SIZE, NUM_PROJ, WTA_DIM):
  """
  Generate a random population
  """
  population = joblib.Parallel(n_jobs=max_thread, prefer="threads")(
    joblib.delayed(generate_proj)(PN_SIZE, NUM_PROJ, WTA_DIM) for _ in range(POP_SIZE))
...
def fitness(chromosome, prev_fitness):
  """
...
def genetic_alg(POP_SIZE, CROSSOVER_PROB, SELECT_PERCENT, MUTATE_PROB, MUTATE_PROB_VEC, ELITE_CHROMS):
  ...
  population, fitness_list = init_pop(POP_SIZE, PN_SIZE, NUM_PROJ, WTA_DIM)
  ...
  for g in range(MAX_GENERATION):
    ...
    if g > 0:  # do not process this step in the 1st generation, since every fitness = -1
      population, fitness_list = evolve(population, fitness_list, 
                            ELITE_CHROMS, SELECT_PERCENT, POP_SIZE, 
                            CROSSOVER_PROB, MUTATE_PROB, MUTATE_PROB_VEC)
...
def bayesian_optimization(NUM_PROJ, PN_SIZE, WTA_DIM):
  ...
  def evolve_bayes(POP_SIZE, SELECT_PERCENT, CROSSOVER_PROB, ELITE_CHROMS, MUTATE_MAT_VAL, MUTATE_VEC_VAL):
    POP_SIZE = int(POP_SIZE) 
    if POP_SIZE % 2:  # the pop size should be even
      POP_SIZE -= 1  # otherwise after the 1st generation it will decrease by 1
    MUTATE_PROB = NUM_PROJ / PN_SIZE * MUTATE_MAT_VAL
    MUTATE_PROB_VEC = MUTATE_VEC_VAL/WTA_DIM
    ELITE_CHROMS = int(ELITE_CHROMS)
    
    return genetic_alg(POP_SIZE, CROSSOVER_PROB, SELECT_PERCENT, 
                    MUTATE_PROB, MUTATE_PROB_VEC, ELITE_CHROMS)

  optimizer = BayesianOptimization(f=evolve_bayes,
            pbounds={"POP_SIZE": (400, 2000), 'SELECT_PERCENT': (0.1, 0.9), 
      			'CROSSOVER_PROB':(0.3, 0.8), 'ELITE_CHROMS': (2, 10), 
      			'MUTATE_MAT_VAL':(0.2, 0.8), 'MUTATE_VEC_VAL':(0.5, 2)},
            random_state=123,
            verbose=2)
  ...
  dic['mutate_mat_val'] = optimizer.max['params']['MUTATE_MAT_VAL']
  dic['mutate_vec_val'] = optimizer.max['params']['MUTATE_VEC_VAL']
